...
def noise_adv_HST():
    ...
    while i < 1000:
        ...
        fsum = f150 + f200 + f115
        inf = detect_sources(fsum, 2.30, 3)
...
def main():
    ...
    partition = np.linspace(0, 64, 9)[:-1]
    path = 'processed_random_J/' + str(N_sim_2) + '/' + prj + '/'
    telescope = 'JWST'
    ...
    for simulation in range(28):
        ...
        print('box size in each dimension', pf.domain_right_edge[0]/8)
        print('box size in each dimension', pf.domain_right_edge.in_units('kpc')[0]/8)

        # EXTRACTING REDSHIFT AND COMPUTING ANGULAR SIZE OF THE BOX
        redshift = pf.current_redshift
        simulation_name = str(simulation)
        theta_arcsec = (1e3 * pf.domain_right_edge.in_units('kpc')[0]/8) / (D_A(redshift) * 1e6) * arcsec_in_rad

...
def main():
    ...
    partition = np.linspace(0, 256, 17)[:-1]
    path = 'processed_giant/HST/' + prj + '/'
    telescope = 'HST'
    ...
    for simulation in range(N_sim_2, N_sim_2+1):
        ...
        print('box size in each dimension', pf.domain_right_edge[0]/16)
        print('box size in each dimension', pf.domain_right_edge.in_units('kpc')[0]/16)

        # EXTRACTING REDSHIFT AND COMPUTING ANGULAR SIZE OF THE BOX
        redshift = pf.current_redshift
        simulation_name = str(simulation)
        theta_arcsec = (1e3 * pf.domain_right_edge.in_units('kpc')[0]/16) / (D_A(redshift) * 1e6) * arcsec_in_rad

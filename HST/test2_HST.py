...
def init_noise():
    ...
    noise = np.vstack((np.abs(stats.norm.rvs(0,0.00275845,nbins_min*nbins_min)),
                       np.abs(stats.norm.rvs(0,3.26572605e-03,nbins_min*nbins_min)),
                       np.abs(stats.norm.rvs(0,0.00239519,nbins_min*nbins_min))))
...
def init_PSF():
    ...
    #a = fits.open('tinytim/psf_f125w_20arcsec.fits')[0].data
    #b = fits.open('tinytim/psf_f140w_20arcsec.fits')[0].data
    #c = fits.open('tinytim/psf_f160w_20arcsec.fits')[0].data
    ...
    NNbins = int(3/0.13)
    pix_edges  = np.linspace(-3,3,NNbins+1)
    pix_coords = np.linspace(-3+3/NNbins,3-3/NNbins,NNbins)

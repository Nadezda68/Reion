...
def init_input_data(nsim, n_cyl1, n_cyl2, orient=2, thr=3):

    global N_sim_2, telescope, filter_name, prj, N1, N2, threshold
...
def noise_adv_HST():
    ...
    while i < 1000:
        ...
        fsum = f125 + f140 + f160
        inf = detect_sources(fsum, threshold, 3)
...
def main():
    ...
    # LOADING DATA
    N_sim = 0
    files = sorted(glob.glob('/home/kaurov/scratch/rei/code05/OUT_000' + str(N_sim) + str(6) + '_010_080B/rei000'
                             +  str(N_sim) + str(6) + '_010_080B_a0*/rei000' + str(N_sim) + str(6) + '_010_080B_a0.*.art'))

    path = 'processed_huge/' + '17z' + '/'
    telescope = 'HST'
    ...
    for simulation in range(17,18):
        ...
        theta_arcsec = (2 * 100 * 1e3) / (D_A(redshift) * 1e6) * arcsec_in_rad
        ...
        info.write('N of boxes to process: sphere')
        info.write('Theta [arcsec]: %.6f \n' % theta_arcsec)
        info.write('Flux threshold: %.3f \n' % (threshold))
        info.write('Npix: %.3f \n' % npixels)
        info.close()
        ...
        input_coord = [26.6228612866, 30.2283859533, 32.1926369533]
        data = pf.sphere(input_coord, (100, "kpc"))
        # data_sph = pf.sphere([x_0,y_0,z_0], r_0)
        ...
        # m = data_box[('STAR', 'MASS')].in_units('msun')
        ...
        np.savetxt(path + '125.dat', flux_noise_psf125,fmt='%1.5e')
        ...
        np.savetxt(path + '140.dat', flux_noise_psf140,fmt='%1.5e')
        ...
        np.savetxt(path + '160.dat', flux_noise_psf160,fmt='%1.5e')
        flux_noise_psf_std160 = flux_noise_psf160/noise_std160

        flux_noise_psf_std = flux_noise_psf_std160 + flux_noise_psf_std140 + flux_noise_psf_std125
        np.savetxt(path + 'total.dat', flux_noise_psf_std,fmt='%1.5e')
        ...
        for iso_gr, gr_dist, gr_name in zip([0, 1, 1], [3.0, 3.0, 1.0], ['_iso_', '_gr_3_', '_gr_1_']):
            ...
            obj_125_filename = open(path + 'objects' + gr_name + simulation_name + '_125_' + telescope + '.dat', 'ab')
            obj_140_filename = open(path + 'objects' + gr_name + simulation_name + '_140_' + telescope + '.dat', 'ab')
            obj_160_filename = open(path + 'objects' + gr_name + simulation_name + '_160_' + telescope + '.dat', 'ab')
